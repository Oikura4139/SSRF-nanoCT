...
**总损失。** 对当前扫描对象，全部辐射高斯的可学习参数记为：

$$
\theta=\{\rho_i,\mathbf p_i,\mathbf S_i,\mathbf R_i\}_{i=1}^{M},
$$

它们分别对应中心密度、位置、尺度和旋转。每次迭代随机选择一个训练角度，用当前高斯集合生成预测投影 $\mathbf I_r$，并与同角度的测量投影 $\mathbf I_m$ 比较；同时随机体素化一个局部三维密度块 $\mathbf V_{\mathrm{tv}}$。结合论文式（8）和作者公开代码，总损失更准确地写为：

$$
\mathcal L_{\mathrm{total}}(\theta)
=\mathcal L_1(\mathbf I_r,\mathbf I_m)
+0.25\left[1-\operatorname{SSIM}(\mathbf I_r,\mathbf I_m)\right]
+0.05\,\operatorname{TV}_{\mathrm{3D}}(\mathbf V_{\mathrm{tv}}).
$$

这里的 $\mathbf I_m$ 默认是对数变换后的投影，其像素近似表示沿射线的衰减系数积分，而不是原始探测器光子计数；$\mathbf V_{\mathrm{tv}}$ 是当前高斯密度场的预测值，不是真实三维体标签。三项损失的分工如下：

- **投影 $L_1$ 损失：约束绝对线积分。** 作者实现为全部投影像素绝对误差的平均：

  $$
  \mathcal L_1
  =\frac{1}{HW}\sum_{u,v}
  \left|I_r(u,v)-I_m(u,v)\right|.
  $$

  它直接要求每条射线的预测衰减量接近测量值，是恢复定量密度尺度的主要数据一致性项。仅使用结构相似性可能允许整体强度或对比度偏移，因此不能替代该项。

- **结构相异性损失：约束局部投影结构。** 论文将其称为 D-SSIM。作者代码实际采用 $1-\operatorname{SSIM}$，而不是直接最小化相似度，也不是部分实现中的二分形式。其 $11\times11$ 高斯窗口的标准差为 $\sigma=1.5$，用于比较局部均值、方差和协方差，因而对轮廓、边缘及局部衰减分布比逐像素 $L_1$ 更敏感。投影绝对误差回答“数值是否正确”，结构相异性损失回答“局部结构是否相似”。

- **三维 TV 损失：约束局部密度振荡。** 作者实现的是沿三个坐标方向计算一阶绝对差的各向异性三维 TV：

  $$
  \operatorname{TV}_{\mathrm{3D}}(\mathbf V)
  =\frac{
  \sum|\Delta_x\mathbf V|+
  \sum|\Delta_y\mathbf V|+
  \sum|\Delta_z\mathbf V|
  }{3D^2(D-1)}.
  $$

  每轮只查询一个与最终输出体素间距相同的立方形局部块，边长设置为 $D=32$，而不是对完整 $256^3$ 体数据计算总变分。它不依赖三维真值，而是引入“材料或组织内部通常局部均匀”的先验，在稀疏投影存在多解时抑制条纹、孤立噪点和针状密度振荡。绝对差形式通常比平方梯度更有利于保留密度边界，但仍存在过度平滑风险；后文消融显示，过大的 TV 权重会降低重建指标。

论文正文只给出三项损失的名称与总式；上述 $1-\operatorname{SSIM}$、局部窗口和 TV 离散归一化来自作者公开的[训练代码](https://github.com/Ruyi-Zha/r2_gaussian/blob/main/train.py#L117-L144)与[损失函数实现](https://github.com/Ruyi-Zha/r2_gaussian/blob/main/r2_gaussian/utils/loss_utils.py#L19-L104)。三项损失相加后只进行一次反向传播。投影 $L_1$ 和 D-SSIM 的梯度经过可微 X 射线光栅化器回到高斯参数，三维 TV 的梯度经过可微体素化器回到同一组参数：

$$
\nabla_\theta\mathcal L_{\mathrm{total}}
=\nabla_\theta\mathcal L_1
+0.25\nabla_\theta\mathcal L_{\mathrm{D\text{-}SSIM}}
+0.05\nabla_\theta\mathcal L_{\mathrm{tv}}.
$$

因此，这一目标可以概括为“用 $L_1$ 保证射线积分数值正确，用 D-SSIM 保证二维局部结构相似，再用 TV 从投影数据允许的多个三维解中偏向局部更平滑的密度场”。系数 $1$、$0.25$ 和 $0.05$ 只是不同损失的乘法权重，不代表三项分别占总损失的固定百分比；其实际影响还取决于各项的数值尺度和梯度大小。

**优化与自适应控制。** 位置、密度、尺度和旋转的初始学习率分别为 0.0002、0.01、0.005 和 0.001，并指数衰减到初始值的 0.1 倍。自适应控制不是额外的损失项：它删除空高斯，对高梯度高斯进行克隆或分裂；考虑到器官具有大面积均匀区域，不删除尺寸大的高斯。新增高斯时将原高斯和复制高斯的密度都减半，以减少基元数量突然变化造成的性能震荡。
